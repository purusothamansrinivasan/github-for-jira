...
func DeleteworkflowtransitionpropertyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteworkflowtransitionpropertyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_rest_api_3_workflow_transitions_transitionId_properties",
		mcp.WithDescription("Delete workflow transition property"),
		...
		mcp.WithString("key", mcp.Required(), mcp.Description("The name of the transition property to delete, also known as the name of the property.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleteworkflowtransitionpropertyHandler(cfg),
	}
}
