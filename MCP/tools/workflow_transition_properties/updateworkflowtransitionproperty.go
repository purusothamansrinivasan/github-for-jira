...
func UpdateworkflowtransitionpropertyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateworkflowtransitionpropertyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_rest_api_3_workflow_transitions_transitionId_properties",
		mcp.WithDescription("Update workflow transition property"),
		...
		mcp.WithString("key", mcp.Required(), mcp.Description("The key of the property being updated, also known as the name of the property. Set this to the same value as the `key` defined in the request body.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateworkflowtransitionpropertyHandler(cfg),
	}
}
