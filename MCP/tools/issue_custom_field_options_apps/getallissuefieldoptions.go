...
func GetallissuefieldoptionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/field/%s/option%s", cfg.BaseURL, fieldKey, queryString)
...
func CreateGetallissuefieldoptionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_field_fieldKey_option",
		mcp.WithDescription("Get all issue field options"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetallissuefieldoptionsHandler(cfg),
	}
}
