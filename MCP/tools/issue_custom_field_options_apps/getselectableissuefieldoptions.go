...
func GetselectableissuefieldoptionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/field/%s/option/suggestions/edit%s", cfg.BaseURL, fieldKey, queryString)
		...
		var result models.PageBeanIssueFieldOption
...
func CreateGetselectableissuefieldoptionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_field_fieldKey_option_suggestions_edit",
		mcp.WithDescription("Get selectable issue field options"),
		...
		mcp.WithNumber("projectId", mcp.Description("Filters the results to options that are only available in the specified project.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetselectableissuefieldoptionsHandler(cfg),
	}
}
