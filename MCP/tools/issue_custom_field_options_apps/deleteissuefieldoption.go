...
func DeleteissuefieldoptionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteissuefieldoptionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_rest_api_3_field_fieldKey_option_optionId",
		mcp.WithDescription("Delete issue field option"),
		...
		mcp.WithNumber("optionId", mcp.Required(), mcp.Description("The ID of the option to be deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteissuefieldoptionHandler(cfg),
	}
}
