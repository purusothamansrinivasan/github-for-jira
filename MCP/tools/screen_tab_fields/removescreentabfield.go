...
func RemovescreentabfieldHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/screens/%s/tabs/%s/fields/%s", cfg.BaseURL, screenId, tabId, id)
...
func CreateRemovescreentabfieldTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_rest_api_3_screens_screenId_tabs_tabId_fields_id",
		mcp.WithDescription("Remove screen tab field"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    RemovescreentabfieldHandler(cfg),
	}
}
