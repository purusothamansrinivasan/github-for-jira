...
func GetallscreentabsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/screens/%s/tabs%s", cfg.BaseURL, screenId, queryString)
...
func CreateGetallscreentabsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_screens_screenId_tabs",
		mcp.WithDescription("Get all screen tabs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetallscreentabsHandler(cfg),
	}
}
