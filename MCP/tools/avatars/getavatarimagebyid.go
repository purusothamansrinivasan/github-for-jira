...
func GetavatarimagebyidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/universal_avatar/view/type/%s/avatar/%s%s", cfg.BaseURL, type, id, queryString)
...
func CreateGetavatarimagebyidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_universal_avatar_view_type_type_avatar_id",
		mcp.WithDescription("Get avatar image by ID"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetavatarimagebyidHandler(cfg),
	}
}
