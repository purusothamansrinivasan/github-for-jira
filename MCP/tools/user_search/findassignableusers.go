...
func FindassignableusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["recommend"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("recommend=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/rest/api/3/user/assignable/search%s", cfg.BaseURL, queryString)
...
func CreateFindassignableusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_user_assignable_search",
		mcp.WithDescription("Find users assignable to issues"),
		...
		mcp.WithString("sessionId", mcp.Description("The sessionId of this request. SessionId is the same until the assignee is set.")),
		...
		mcp.WithString("project", mcp.Description("The project ID or project key (case sensitive). Required, unless `issueKey` is specified.")),
		mcp.WithString("issueKey", mcp.Description("The key of the issue. Required, unless `project` is specified.")),
		mcp.WithNumber("startAt", mcp.Description("The index of the first item to return in a page of results (page offset).")),
		mcp.WithNumber("maxResults", mcp.Description("The maximum number of items to return. This operation may return less than the maximum number of items even if more are available. The operation fetches users up to the maximum and then, from the fetched users, returns only the users that can be assigned to the issue.")),
		mcp.WithNumber("actionDescriptorId", mcp.Description("The ID of the transition.")),
		mcp.WithBoolean("recommend", mcp.Description("")),
	)
