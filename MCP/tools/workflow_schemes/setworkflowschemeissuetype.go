...
func SetworkflowschemeissuetypeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/workflowscheme/%s/issuetype/%s", cfg.BaseURL, id, issueType)
...
func CreateSetworkflowschemeissuetypeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_rest_api_3_workflowscheme_id_issuetype_issueType",
		mcp.WithDescription("Set workflow for issue type in workflow scheme"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    SetworkflowschemeissuetypeHandler(cfg),
	}
}
