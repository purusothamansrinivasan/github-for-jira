...
func SetissuetypepropertyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateSetissuetypepropertyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_rest_api_3_issuetype_issueTypeId_properties_propertyKey",
		mcp.WithDescription("Set issue type property"),
		mcp.WithString("issueTypeId", mcp.Required(), mcp.Description("The ID of the issue type.")),
		mcp.WithString("propertyKey", mcp.Required(), mcp.Description("The key of the issue type property. The maximum length is 255 characters.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    SetissuetypepropertyHandler(cfg),
	}
}
