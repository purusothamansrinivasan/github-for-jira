...
func GetoptionsforcontextHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["optionId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("optionId=%v", val))
		}
		if val, ok := args["onlyOptions"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("onlyOptions=%v", val))
		}
		...
		url := fmt.Sprintf("%s/rest/api/3/field/%s/context/%s/option%s", cfg.BaseURL, fieldId, contextId, queryString)
		...
		var result models.PageBeanCustomFieldContextOption
...
func CreateGetoptionsforcontextTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_field_fieldId_context_contextId_option",
		mcp.WithDescription("Get custom field options (context)"),
		...
		mcp.WithNumber("optionId", mcp.Description("The ID of the option.")),
		mcp.WithBoolean("onlyOptions", mcp.Description("Whether only options are returned.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetoptionsforcontextHandler(cfg),
	}
}
