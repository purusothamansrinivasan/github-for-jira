...
func UpdatefilterHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/filter/%s%s", cfg.BaseURL, id, queryString)
...
func CreateUpdatefilterTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_rest_api_3_filter_id",
		mcp.WithDescription("Update filter"),
		mcp.WithNumber("id", mcp.Required(), mcp.Description("The ID of the filter to update.")),
		...
		mcp.WithBoolean("overrideSharePermissions", mcp.Description("EXPERIMENTAL: Whether share permissions are overridden to enable the addition of any share permissions to filters. Available to users with *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatefilterHandler(cfg),
	}
}
