...
func GetissueworklogHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["startedAfter"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("startedAfter=%v", val))
		}
		if val, ok := args["startedBefore"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("startedBefore=%v", val))
		}
		...
		var result models.PageOfWorklogs
...
func CreateGetissueworklogTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_issue_issueIdOrKey_worklog",
		mcp.WithDescription("Get issue worklogs"),
		...
		mcp.WithNumber("startedAfter", mcp.Description("The worklog start date and time, as a UNIX timestamp in milliseconds, after which worklogs are returned.")),
		mcp.WithNumber("startedBefore", mcp.Description("The worklog start date and time, as a UNIX timestamp in milliseconds, before which worklogs are returned.")),
