...
func GetworkflowschemedraftissuetypeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetworkflowschemedraftissuetypeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_workflowscheme_id_draft_issuetype_issueType",
		mcp.WithDescription("Get workflow for issue type in draft workflow scheme"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetworkflowschemedraftissuetypeHandler(cfg),
	}
}
