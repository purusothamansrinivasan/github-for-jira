...
func CreateprojectavatarHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/project/%s/avatar2%s", cfg.BaseURL, projectIdOrKey, queryString)
...
func CreateCreateprojectavatarTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_rest_api_3_project_projectIdOrKey_avatar2",
		mcp.WithDescription("Load project avatar"),
		...
		mcp.WithNumber("size", mcp.Description("The length of each side of the crop region.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateprojectavatarHandler(cfg),
	}
}
