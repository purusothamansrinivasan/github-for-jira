...
func GetworklogpropertykeysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/issue/%s/worklog/%s/properties", cfg.BaseURL, issueIdOrKey, worklogId)
...
func CreateGetworklogpropertykeysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_issue_issueIdOrKey_worklog_worklogId_properties",
		mcp.WithDescription("Get worklog property keys"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetworklogpropertykeysHandler(cfg),
	}
}
