...
func RemovewatcherHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/issue/%s/watchers%s", cfg.BaseURL, issueIdOrKey, queryString)
...
func CreateRemovewatcherTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_rest_api_3_issue_issueIdOrKey_watchers",
		mcp.WithDescription("Delete watcher"),
