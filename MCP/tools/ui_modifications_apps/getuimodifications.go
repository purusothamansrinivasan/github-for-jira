...
func GetuimodificationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/uiModifications%s", cfg.BaseURL, queryString)
		...
		var result models.PageBeanUiModificationDetails
...
func CreateGetuimodificationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_uiModifications",
		mcp.WithDescription("Get UI modifications"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetuimodificationsHandler(cfg),
	}
}
