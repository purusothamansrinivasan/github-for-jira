...
func GetallissuetypeschemesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/issuetypescheme%s", cfg.BaseURL, queryString)
		...
		var result models.PageBeanIssueTypeScheme
...
func CreateGetallissuetypeschemesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_issuetypescheme",
		mcp.WithDescription("Get all issue type schemes"),
		...
		mcp.WithArray("id", mcp.Description("The list of issue type schemes IDs. To include multiple IDs, provide an ampersand-separated list. For example, `id=10000&id=10001`.")),
		...
		mcp.WithString("queryString", mcp.Description("String used to perform a case-insensitive partial match with issue type scheme name.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetallissuetypeschemesHandler(cfg),
	}
}
