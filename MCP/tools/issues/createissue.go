...
func CreateissueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/issue%s", cfg.BaseURL, queryString)
		...
		var result models.CreatedIssue
...
func CreateCreateissueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_rest_api_3_issue",
		mcp.WithDescription("Create issue"),
		mcp.WithBoolean("updateHistory", mcp.Description("Whether the project in which the issue is created is added to the user's **Recently viewed** project list, as shown under **Projects** in Jira. When provided, the issue type and request type are added to the user's history for a project. These values are then used to provide defaults on the issue create screen.")),
