...
func GetchangelogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/issue/%s/changelog%s", cfg.BaseURL, issueIdOrKey, queryString)
		...
		var result models.PageBeanChangelog
...
func CreateGetchangelogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_issue_issueIdOrKey_changelog",
		mcp.WithDescription("Get changelogs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetchangelogsHandler(cfg),
	}
}
