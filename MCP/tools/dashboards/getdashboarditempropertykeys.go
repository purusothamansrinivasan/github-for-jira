...
func GetdashboarditempropertykeysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/rest/api/3/dashboard/%s/items/%s/properties", cfg.BaseURL, dashboardId, itemId)
...
func CreateGetdashboarditempropertykeysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_rest_api_3_dashboard_dashboardId_items_itemId_properties",
		mcp.WithDescription("Get dashboard item property keys"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetdashboarditempropertykeysHandler(cfg),
	}
}
